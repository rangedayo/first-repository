...
app.use(cors({
    ...
    credentials: true,
    // file:// 프로토콜과 로컬 개발을 위해 모든 origin 허용 (개발 환경에서만)
...
class PlantIdService {
    ...
    async identifyPlant(base64Image) {
        ...
        // Plant.id API v3 - 가장 기본적인 형태로 단순화
        const requestData = {
            images: [base64Image], // 순수 base64 문자열만
            similar_images: true   // 가장 기본적인 modifier만
        };
...
class ResultProcessor {
    ...
    static generateTreatments(diseases) {
        ...
        return treatments.filter((treatment, index, self) => 
...
app.get('/api/plant-id-status', async (req, res) => {
    try {
        ...
        if (!plantService.apiKey) {
            return res.json({
                status: 'error',
                message: 'Plant.id API 키가 설정되지 않았습니다.'
            });
        }
        
        res.json({
            status: 'ok',
            message: 'Plant.id API 키가 설정되어 있습니다.'
        });
...
app.listen(PORT, () => {
    console.log('\n🌱 Plant Diagnosis Server (Kindwise Plant.id)');
